...
echo 'History Length,Forecast Length,Units,Epochs,Interval,Loss' >> offline-results.csv
  ...
  for l in {50..500..50}; do
    ...
    for u in {20..200..20}; do
      ...
      for e in {1..10}; do
        ((i = i % 100))
        ((i++ == 0)) && wait
        srun -n 1 -c 1 --exclusive offline-run.sh -f $f -l $l -e $e -u $u >> offline-results.csv &
      done
    done
  done
done
